...
int getKey()
{
	...
	while (1)
	{
		ReadConsoleInput(hin, &ipRec, 1, &dwRead);
		if (ipRec.EventType == KEY_EVENT)
		{
			keyRec = ipRec.Event.KeyEvent;
			if (keyRec.bKeyDown)
				return keyRec.wVirtualKeyCode ? keyRec.wVirtualKeyCode : toupper(keyRec.uChar.AsciiChar);
		}
	}
}*/
